...
const search = (query) => {
  ...
  let items2;
  items2 = most.items.map((items) => ({ ...items, id: items.id.videoId }));
  console.log(items2);
  console.log(items.length); //25
  videoList.push(`<ul class ="videos">`);
  items2.map((video) => {
    //25번 반복 - 25개의 li
    //자바스크립트에서는 파라미터로 값을 넘길 때 반드시 싱글쿼터에션 또는 더블쿼테이션을 붙여야 값으로 인정됨
    //만일 붙이지 않으면 XXX is noe defined라고 뜬다. - 결론은 변수 취급이다
